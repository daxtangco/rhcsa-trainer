...
- ~~The local repo is ISO-backed and **copied into the VM's own disk**, not a host mount. A host-mounted ISO disappears if the `.vmx` CD-ROM device is detached, and then every `dnf` in every task fails.~~
  ...
  **SUPERSEDED 2026-09-02 — do not implement the copy-in.** The arithmetic does not work: the RHEL 9.8 DVD is 14.47 GiB, and in the built VM `/var` has 1.8 GiB free, `/` has 9.8 GiB, and the whole VG has 15.00 GiB `VFree`. There is nowhere to put it, and carving out space would consume the exact free extents the bullet above reserves for the Phase 1 LVM tasks — the two design notes are in direct conflict, and the LVM one wins because it is an exit criterion.

  The detachment risk this bullet names is real but much smaller than it reads: it is a one-line `vmx` setting the user does not touch between sessions, and the mount is `nofail`, so a detached disc costs a `dnf` failure with a clear warning rather than an unbootable VM. Every code block below that copies the ISO or writes `rhcsa-local.repo` is superseded by the DVD-backed repo in `scripts/guest-provision.sh` — see commit `03e7e4d` for the implementation and its live verification.
